...
<script type="text/javascript">
            ...
            $(document).ready(function () {
                /** 新增   **/
                $("#addBtn").fancybox({
                	'href': 'punchCard_add.jsp?page='+${pageBean.totalPage},
                    'width': 733,
                    'height': 530,
                    'type': 'iframe',
                    ...
                    'onClosed': function () {
                        window.location.href = 'punchCard_list.jsp';
                    }
                });
            ...
            function search() {
                var pclock; 
                pclock = document.getElementById("inputValue");
                	$("#submitForm").attr("action", "PunchCardServlet?method=search&pclock="+ pclock.value).submit();
            }

            /** 导出 **/
            function exportExcel(){
            	if( confirm('您确定要导出吗？') ){
            		$("#submitForm").attr("action", "ExportServlet?object=punchCard").submit();
            	}
            }
            ...
            function batchDel() {
                ...
                if (allIDCheck.length > 0) {
                    ...
                    if (confirm("您确定要批量删除这些记录吗？")) {
                        // 提交form
                        $("#submitForm").attr("action", "PunchCardServlet?method=deleteAll&page="+${pageBean.currentPage}).submit();
                    }
                }
            ...
            function jumpNormalPage(page) {
                $("#submitForm").attr("action", "PunchCardServlet?page=" + page).submit();
            } 
            ...
            function jumpInputPage(totalPage) {
                ...
                if ($("#jumpNumTxt").val() != '') {
                    ...
                    $("#submitForm").attr("action", "PunchCardServlet?page=" + pageNum).submit();
                } else {
                    // “跳转页数”为空
                    art.dialog({icon: 'error', title: '友情提示', drag: false, resize: false, content: '请输入合适的页数，\n自动为您跳到首页', ok: true, });
                    $("#submitForm").attr("action", "PunchCardServlet?page=" + 1).submit();
                }
            }
        </script>
<style>
...
<body>
	<form id="submitForm" name="submitForm" action="PunchCardServlet" method="post">
			...
			<div class="ui_content">
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_center">
							<input type="text" id="inputValue" name="punchCard" class="ui_input_txt02" placeholder="打卡人编号"/>
				...
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${list}" var="user">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="${user.pId}" class="acb" /></td>
								<td><c:out value="${user.pId}" /></td>
								<td><c:out value="${user.pclock}" /></td>
								<td><c:out value="${user.pnote}" /></td>
								<td><c:out value="${user.ptime}" /></td>
								<td><%-- <a href="punchCard_edit.jsp?pclock=${user.pclock}&pnote=${user.pnote}&ptime=${user.ptime}&page=${pageBean.currentPage}" class="edit" id="addBtn">编辑</a> --%> 
								<a href="PunchCardServlet?method=delete&pId=${user.pId}&page=${pageBean.currentPage}">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</div>
